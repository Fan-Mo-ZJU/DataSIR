    ...
    def transform(self, data: str) -> str:
        ...
        if s.isdigit():
            try:
                ...
                binary_parts = [bin(int(char))[2:].zfill(4)[-4:] for char in s]  # 去除0b前缀并填充至4位，只保留后四位
...
class OctalTransform(BaseFormatTransform):
    def transform(self, data: str)-> str:
        ...
        if s.isdigit():
            try:
                octal_parts = [oct(int(char))[2:] for char in s]  # 去除0o前缀
...
class ASCIITransform(BaseFormatTransform):  #只转换数字、字母、符号
    def transform(self, data: str)-> str:
        results = []
        for char in data:
            code = ord(char)
...
class URLTransform(BaseFormatTransform):  #只转换汉字和特殊字符
    def transform(self, data: str)-> str:
...
class MorseTransform(BaseFormatTransform): #只转换数字、字母、符号
    def transform(self, data: str)-> str:
...
class BrailleTransform(BaseFormatTransform):  #只转换数字、字母、符号
    def transform(self, data: str)-> str:
...
class CharacterDecompositionTransform(BaseFormatTransform):  #只转换汉字
    def transform(self, data: str)-> str:
...
class MartianTextTransform(BaseFormatTransform):   #只转换汉字、数字、字母
    def transform(self, data: str)-> str:
        result = []
        for char in data:
            # 处理优先级：中文 > 英文 > 数字
...
class SimplifiedToTraditionalChineseTransform(BaseFormatTransform):  #只转换汉字
    def transform(self, data: str)-> str:
        result = []
        for char in data:
            if is_chinese_char(char):
...
class NumericalCapitalizationTransform(BaseFormatTransform):  #只转换数字
    def transform(self, data: str)-> str:
...
class InsertingChineseCharactersTransform(BaseFormatTransform):  #只转换数字和字母
    def transform(self, data: str)-> str:
        # 找出所有英文/数字字符的位置
...
class InsertingEnglishLettersNumbersTransform(BaseFormatTransform):  #只转换汉字
    def transform(self, data: str)-> str:
